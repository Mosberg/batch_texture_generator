...
import tools.btg_v2 as btg_v2


class TkLogHandler(btg_v2.logging.Handler):
    def __init__(self, q: "queue.Queue[str]") -> None:
        super().__init__()
        self._q = q

    def emit(self, record: btg_v2.logging.LogRecord) -> None:
        try:
...
class App(ttk.Frame):
    ...
    def _setup_logging(self) -> None:
        ...
        handler.setFormatter(btg_v2.logging.Formatter("%(levelname)s: %(message)s"))

        btg_v2.LOG.setLevel(btg_v2.logging.INFO)
        btg_v2.LOG.handlers.clear()
        btg_v2.LOG.addHandler(handler)

        root = btg_v2.logging.getLogger()
        root.setLevel(btg_v2.logging.INFO)
        root.handlers.clear()
